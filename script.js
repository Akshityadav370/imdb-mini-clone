...
async function loadMovies(searchTerm) {
    const URL = `http://www.omdbapi.com/?s=${searchTerm}&page=1&apikey=1f05069a`; //Base URL
...
const loadMovieDetails = () => {
    ...
    searchListMovies.forEach(movie => {
        movie.addEventListener('click', async () => {
            ...
            let dir = window.location.origin + "/result/result.html"; // Custom URL for result page
            window.location.href = "./result/result.html"; //Redirect to a new page
        })
    })
}
